...
def pick_a_cor():
  ...
  x = random.uniform(0, 34.0867)
  y = random.uniform(0, 19.68)
  z = random.uniform(0, 60)
  #z = random.choice([random.uniform(0, 8.44), random.uniform(25.44, 100)])
...
def gen_add_info(atoms,cor_candidate,eles_pool,natm_to_add=1):
  '''
  ...
  for i in range(natm_to_add):
    ...
    ele_pick = random.choice(eles_pool)
    ele_lpj = list(type_map).index(ele_pick) + 1
